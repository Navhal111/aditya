...
,{headers:headers})
 ...
 .map( res => console.log('is',res.json()));
  ...
  submitFormX (form): Observable<any> {
          ...
          return this._http.get(`http://192.168.1.104:3000/complaint/create?subject=`+form.subject+`&description=`+form.description+`&image=`+imageLink+`&latitude=12.45&longitude=21.54&address=`+form.address+`&district=`+form.district+`&state=`+form.state+`&pincode=`+form.pincode,{headers:headers}) // ...using post request
    ...
    passwordChange(form):Observable<any>{
      ...
      return this._http.get(`http://192.168.1.104:3000/user/update_password?old_password=`+form.oldpass+`&new_password=`+newP,{headers:headers}) // ...using post request
                       .map(res=> res.json()) // ...and calling .json() on the response to return data
                       .catch((error:any) => Observable.throw(error.json().error || 'Server error')); //...errors if any
    }
    ...
    verifyAadhar(form):Observable<any>{
      ...
      var secret = window.localStorage.getItem('secret_key')
       ...
       headers.append('secret_key',secret)
       headers.append('Content-Type','application/json');
      return this._http.get(`http://192.168.1.104:3000/aadhar_verification/verify_aadhar_data?aadhar_number=`+form.aadhar_number+`&contact=`+form.contact,{headers:headers}) // ...using post request
                       .map(res=> res.json()) // ...and calling .json() on the response to return data
                       .catch((error:any) => Observable.throw(error.json().error || 'Server error')); //...errors if any
    }
   ...
   verifyOTP(form):Observable<any>{
     ...
     var secret = window.localStorage.getItem('secret_key')
      ...
      headers.append('secret_key',secret)
      headers.append('Content-Type','application/json');
     return this._http.get(`http://192.168.1.104:3000/aadhar_verification/verify_otp?otp=`+form._otp,{headers:headers}) // ...using post request
                      .map(res=> res.json()) // ...and calling .json() on the response to return data
                      .catch((error:any) => Observable.throw(error.json().error || 'Server error')); //...errors if any
   }
