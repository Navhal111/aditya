...
export class AppService {
 ...
 loginFun(usercreds):Promise<any>{
  ...
  return new Promise ((resolve) => {
   ...
   this._http.post(`http://192.168.1.104:3000/auth/user_login?email=`+usercreds.emailId+`&password=`+usercreds.password,{headers:headers})
   .map( res => res.json())
     .subscribe((res) =>{
...
//this._http.post(`http://192.168.1.104:3000/user/signup?name=`+usercreds.name+`&contact=`+usercreds.contact+`&email=`+usercreds.emailId+`&password=`+usercreds.password,{headers:headers})
...
getStatusX(){
 this._http.get(`http://192.168.1.104:3000/complaint/show_user_complaints`,{headers:this.getHeaders()})
...
adminFetchComp(){
return this._http.get(`http://192.168.1.104:3000/admin_user/fetch_complaints`,{headers:this.getAdminHeaders()})
       .map( res => res.json())
}
  ...
  getStats(){
    return this._http.get(`http://192.168.1.104:3000/admin_user/fetch_statistics`,{headers:this.getAdminHeaders()})
          .map(res=>res.json())
  }


  getDetailsComp(param){
    this.fetchUrl = 'http://192.168.1.104:3000/complaint/show_complaint_by_id?id='+param
